...
\textbf{Details:} \\[6pt]
\textbf{Input:} \\
$\prod_{i=1}^k d_i$: Eingabedimension = Produkt Achsen-Dimensionen \\[4pt]
...
\noindent{\scriptsize
Siehe Hinweis auf den nächsten Seiten von Architekturdarstellungen aus Platzgründen.
}
...
\textbf{CNN-Backbone:} \\
Architektur: z. B. ResNet-50 (Amini 2020 \parencite{amini2020deep})\\
...
\noindent{\scriptsize
Die Übersichten wurden zu einer der Recherche begleitenden Reflektion ausgearbeitet und haben keinen Vollständigkeitsanspruch, insbesondere in Bezug auf Quellen oder Eingabedimensionen, die nicht zur Trainingsparametrisierung der Durchführung passen. Es wurden auch Konfigurationen mit Eingaben getestet, die nicht in der Durchführung enthalten oder für diese relevant sind.
}
...
%\needspace{10\baselineskip}
\subsection*{\gls{Conformal Prediction} mit \gls{randomforestregression} nach Angelopoulos \parencite{angelopoulos2021gentle}}
...
\adjustbox{valign=t, scale=0.8}{
    ...
    \node[layer, fill=green!15, below=of input] (forest) {Random Forest Regressor \\ $T$ Bäume};

    \node[layer, fill=orange!20, below=of forest] (calib) {Kalibrierung \\ Residuen $\hat{r}_i = |y_i - \hat{y}_i|$};

    \node[layer, fill=yellow!15, below=of calib] (interval) {Konfidenzintervall \\ $[\hat{y} - q_\alpha, \hat{y} + q_\alpha]$};

    % Arrows
    \draw[arrow] (input) -- (forest);
    \draw[arrow] (forest) -- (calib);
...
$T = 100$ Entscheidungsbäume \\ 
Trainiert auf $\mathcal{D}_{\text{train}}$ \\[4pt]
